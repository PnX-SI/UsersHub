...
$sqliste = "SELECT c.*, u.nom_role, u.prenom_role, u.groupe, b.nom_unite FROM utilisateurs.cor_role_menu c
...
while ($val = pg_fetch_assoc($result)){
	...
	$id_menu = $val['id_menu'];
    $groupe = $val['groupe'];
	$text = "{id_role:".$id_role.",role:'".$role."',groupe:'".$groupe."',id_menu:".$id_menu.",nom_unite:'".$nom_unite."'}";
	$json = $json.$text;
	$i++;
	if ($i > 0 and $i!=$nb) {
		$json = $json. ",";
