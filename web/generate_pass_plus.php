...
foreach ($json as $array) {
    foreach ($array as $database) {
		...
		if ($connect_host<>"" OR $connect_dbname<>"" OR $connect_user<>"" OR $connect_pass<>"") {
            ...
            $sql = "UPDATE utilisateurs.t_roles SET pass_plus = '".$passplus."', pass = '". $passmd5 ."' WHERE id_role = ".$dat['id_role'];
            print $sql;
			$result = pg_query($sql) or die ('{success: false, msg:"ben ! pas bon AAA."}') ;
