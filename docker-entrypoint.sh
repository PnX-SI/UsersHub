...
function schema_exists () {
    ...
    r=`PGPASSWORD=$db_pass psql -X -A -t service=gnlpoaura -c "$q"`
    if [ $r == 't' ]; then
      true
    else 
      false
...
if [ ! -f ./config/config.py ]; then
    echo "# Config generated by docker-entrypoint.sh"
    ...
    echo "ID_APP = 1" >> ./config/config.py
    echo "" >> ./config/config.py
    echo "# Authentification crypting method (hash or md5)" >> ./config/config.py
    echo "PASS_METHOD = '${PASS_METHOD:-hash}'" >> ./config/config.py
    echo "" >> ./config/config.py
    echo "FILL_MD5_PASS=False" >> ./config/config.py
    echo "" >> ./config/config.py
    echo "" >> ./config/config.py
    echo "COOKIE_EXPIRATION = 3600" >> ./config/config.py
    echo "COOKIE_AUTORENEW = True" >> ./config/config.py
    echo "" >> ./config/config.py
    echo "" >> ./config/config.py
