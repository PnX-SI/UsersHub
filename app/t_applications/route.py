...
def applications():
    ...
    contents = TApplications.get_all(order_by="nom_application")
...
def info(id_application):
    ...
    profils = db.session.query(CorProfilForApp).filter(
        CorProfilForApp.id_application == id_application
    )
    return render_template(
        "info_application.html", application=application, users=users, profils=profils
    )


@route.route(
    "application/add/new", defaults={"id_application": None}, methods=["GET", "POST"]
)
...
def addorupdate(id_application):
    ...
    if id_application == None:
        ...
        return render_template(
            "application.html", form=form, title="Formulaire Application"
        )
    else:
        application = TApplications.get_one(id_application)
...
@route.route(
    "application/add/role_profil/<int:id_application>", methods=["GET", "POST"]
)
@route.route(
    "application/update/role_profil/<int:id_role>/<int:id_application>",
    methods=["GET", "POST"],
)
@fnauth.check_auth(
    6,
            ...
            try:
                if id_role:
                    ...
                    CorRoleAppProfil.delete(
                        id_role=id_role, id_application=id_application
                    )
                # et on post
...
@route.route(
    "application/delete/role_profil/<id_role>/<id_application>", methods=["GET", "POST"]
)
@fnauth.check_auth(
    6,
)
